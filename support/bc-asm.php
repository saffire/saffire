...
class Assembler {
    ...
    public function assemble() {
        ...
        foreach ($this->lines as $ln => $line) {
            ...
            # Match opcodes
            if (preg_match_all("/([a-z_]+)\s*(\"[^\"]*\"|\d+|\#[a-z0-9_]+|[a-z0-9_]+)*(?:,\s*(\"[^\"]*\"|\d+|\#[a-z0-9_]+|[a-z0-9_]+))*/i", $line, $matches)) {
                // Parse opcode and operands from source line
                $opcode = $matches[1][0];
                $operand = array();
                $operand[0] = $matches[2][0];
                $operand[1] = isset($matches[3]) ? $matches[3][0] : "";
                ...
                if ($operand_count == 2 && $operand[1] == "") {
                    printf("Opcode ".$opcode." on line ".$this->lineno." requires 2 operands!\n");
                    return null;
                }
                if ($operand_count == 1 && $operand[0] == "") {
                    printf("Opcode ".$opcode." on line ".$this->lineno." requires 1 operand!\n");
                    return null;
                }
                if ($operand_count == 1 && $operand[1] != "") {
                    printf("Opcode ".$opcode." on line ".$this->lineno." requires only 1 operand!\n");
                    return null;
                }
                if ($operand_count == 0 && $operand[0] != "") {
                    printf("Opcode ".$opcode." on line ".$this->lineno." requires no operands!\n");
                    return null;
                }
    ...
    public function c_print($bc) {
        $name = str_replace(".", "_", $this->filename);
        echo "/**\n";
